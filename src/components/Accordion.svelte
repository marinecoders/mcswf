...
<div class="grid grid-cols-1 md:grid-cols-6 md:h-max-content" >
  ...
  <div role="button" tabindex="0" class="col-span-1  h-96 lg:{active==0 ? 'col-span-3': 'col-span-2'} bg-pink-400 lg:h-full" on:click={() => (active=0)}>
    Our Product Portfolio
  </div>
  <!-- svelte-ignore a11y-click-events-have-key-events -->
  {#key active}
  <div role="button" tabindex="0" class="h-fit col-span-1 lg:row-span-1 md:h-full lg:{active==1 ? 'col-span-2': 'col-span-1'} red-radial-gradient" on:click={() => (active=1)} >
  ...
  {/key}
  <!-- svelte-ignore a11y-click-events-have-key-events -->
  <div role="button" tabindex=0 class="h-fit col-span-1 lg:{active==2 ? 'col-span-2': 'col-span-1'} xl:{active==2 ? 'col-span-2': 'col-span-1'} red-radial-gradient" on:click={() => (active=2)}>
    <div class="relative h-full lg:border-x lg:border-y-0 border-y">
    {#if active ==2}
    <AccordionSectionOpen 
  ...
  <!-- svelte-ignore a11y-click-events-have-key-events -->
  <div role="button" tabindex=0 class="h-fit col-span-1 lg:{active==3 ? 'col-span-2': 'col-span-1'} xl:{active==3 ? 'col-span-2': 'col-span-1'} red-radial-gradient" on:click={() => (active=3)}>
    <div class="relative h-full lg:border-x lg:border-y-0 border-y">
      {#if active ==3}
      <AccordionSectionOpen 
